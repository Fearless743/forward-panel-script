#!/bin/bash
# 下载基础地址（根据架构拼接具体文件名）——使用公开仓库以避免私有仓库权限问题
BASE_URL="https://github.com/Fearless743/forward-panel-script/releases/download/gost-latest"
...
detect_arch() {
  ...
  case "$ARCH" in
    x86_64|amd64)
      BINARY_NAME="gost-linux-amd64"
      ;;
    aarch64|arm64)
      BINARY_NAME="gost-linux-arm64"
      ;;
    *)
      echo "⚠️ 未识别的架构: $ARCH，默认使用 amd64 版本"
      BINARY_NAME="gost-linux-amd64"
      ;;
  esac
}
